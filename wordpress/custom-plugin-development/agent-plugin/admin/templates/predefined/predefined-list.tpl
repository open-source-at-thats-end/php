				...
				{*{foreach $rsPredefined as $record}
					<tr>
						...
						<td width="">
							...
							<a href="{$scriptname}&action=edit&pk={$record['psearch_id']}"  id="a_edit" ><b>Edit</b></a>&nbsp;&nbsp;
							<a href="javascript:void(0);"  id="a_delete" onclick="JavaScript:CDelete_Click('{$scriptname}', '{$record['psearch_id']}', '');"><b>Delete</b></a>&nbsp;&nbsp;
						</td>
					</tr>
				{/foreach}*}
				{while $rsPredefined->next_record()}
					<tr>
						<td width="">{$rsPredefined->f('psearch_title')}</td>
						<td width="">{include file='predefined/search_criteria.tpl' arrSearchResult=$rsPredefined->f('psearch_criteria')|unserialize}</td>
{*						<td width="">{$objAdminPre->getPropertyCount($rsPredefined->f('psearch_id'))|number_format:0}</td>*}
						<td width="">{$rsPredefined->f('psearch_tag')}</td>
						<td width="">[predefine-search-result ag=1 pid={$rsPredefined->f('psearch_id')}]</td>
						<td width="">
							<a href="JavaScript: void(0);" class="vlink_listing" data-link="{$scriptname}&action=view&psearch_id={$rsPredefined->f('psearch_id')}" id="a_view" ><b>View Listing</b></a>&nbsp;&nbsp
							<a href="{$scriptname}&action=edit&pk={$rsPredefined->f('psearch_id')}"  id="a_edit" ><b>Edit</b></a>&nbsp;&nbsp;
							<a href="javascript:void(0);"  id="a_delete" onclick="JavaScript:CDelete_Click('{$scriptname}', '{$rsPredefined->f('psearch_id')}', '');"><b>Delete</b></a>&nbsp;&nbsp;
