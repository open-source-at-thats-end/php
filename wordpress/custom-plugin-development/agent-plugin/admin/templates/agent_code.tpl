...
<form id="FrmAgentInfo" method="get" action="">
	...
	<div class="row">
		<div class="span6">
			<label><b>ID</b></label>
			<input type="text" id="agent_first_name" name="agent_id" value="{$arrParams.agent_id|default:''}" class="input-lg for-search"/>
		</div>
		<div class="span6">
			<label><b>Name</b></label>
			<input type="text" id="agent_last_name" name="agent_name" value="{$arrParams.agent_name|default:''}" class="input-lg for-search"/>
		</div>
		<div class="span9">
    ...
    <div class="-vlist-data" id="agent-list-holder">
        <div class="main-">
            <table class="table table-bordered  table-condensed table-hover  table-striped">
                <tbody id="the-list">
                {if $total_record > 0}
                    {while $rsAgCode->next_record()}
                        <tr>
                            <td width="15%">{$rsAgCode->f('Agent_ID')}</td>
                            <td width="30%">{$rsAgCode->f('Agent_FName')} {$rsAgCode->f('Agent_LName')}</td>
                            <td width="30%">{$rsAgCode->f('Office_Name')}</td>
                        </tr>
                    {/while}
                {else}
