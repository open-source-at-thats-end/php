...
<form id="frmSettings" action="" method="post" enctype="multipart/form-data">
    <div class="tab-content" style="overflow: unset;">
            ...
            <div class="row-fluid">
                <div class="span8">
                    {*<div>
                        ...
                        <input type="text" name="agent_sys_name" value="{if isset($AgentSystemName) && $AgentSystemName == {constant('Constants::ACTRIS')}}ACTRIS{else}MAR/BEACHES{/if}" readonly/>
                    </div>*}
                    <div>
                        <label>Name</label>
                    ...
                    <div>
                        ...
                        <input type="text" id="agent_office" name="AgentConfig[agent_office]" minlength="5" maxlength="15" value="{$arrAgentConfig.agent_office}" class="input-xxlarge "/>
                        <label class="error" for="agent_office">Enter Your Office Number</label>
                    </div>
                    <div>
                        <label>About You</label>
                        <textarea id="agent_about" name="AgentConfig[agent_about]" class="input-xxlarge">{$arrAgentConfig.agent_about}</textarea>
                        <label class="error" for="agent_about">Enter something about you</label>
                    </div>
                    <div>
                        <label>Address</label>
                        <textarea id="agent_address" name="AgentConfig[agent_address]" class="input-xxlarge">{$arrAgentConfig.agent_address}</textarea>
                        <label class="error" for="agent_address">Enter Your Address</label>
                    </div>
                    <div>
                        <label>Photo</label>
                        <input type="file" name="agent_photo" id="agent_photo" class="input-xlarge" value="{$arrAgentConfig.agent_photo}" /><br/><br/>
                        {if $arrAgentConfig.agent_photo != ''}
                            <img class="agent-photo" src="{$uploadPath}{$arrAgentConfig.agent_photo}" width="90"/>
                    ...
                    <div class="">
                        ...
                        <input type="file" name="print_photo" id="print_photo" class="input-xlarge" /><br/><br/>
                        {if $arrAgentConfig.print_photo != ''}
                            <img class="agent-logo" src="{$uploadPath}{$arrAgentConfig.print_photo}" width="90"/>
                            <br><input type="checkbox" id= "del_logo" tabindex="{$ControlTabIndex}" name="del_logo" value="1" class="stdCheckbox"> Delete Logo
                            <input type="hidden" name="prev_print_photo" value="{$arrAgentConfig.print_photo}" />
                        {/if}
                    </div>
                    <br><input type="checkbox" id= "not_display_info" style="margin-top: 0px" tabindex="{$ControlTabIndex}" name="AgentConfig[not_display_info]" value="Yes" class="" {if isset($arrAgentConfig.not_display_info) && $arrAgentConfig.not_display_info == 'Yes'}checked="checked"{/if}> Hide Profile Information on Property Detail ?
                    <div><label>&nbsp;</label></div>
                </div>
        ...
        <div id="tab2" class="tab-pane well wellmedpadd">
            <div class="row-fluid">
                ...
                <div class="span8">
                    ...
                    {/if}
                    <div>
                        <label>Require Registration for Development Page. </label>
                        <select id="require_reg_for_dev_page" name="Listing[require_reg_for_dev_page]">
                            {foreach from=$arrYesNo key=key item=item}
                                <option value="{$key}" {if $key == $arrListingConfig['require_reg_for_dev_page']}selected {elseif $arrListingConfig['require_reg_for_dev_page'] == '' && $key == 'No'}selected{/if}>{$item}</option>
                            {/foreach}
                        </select>
                    </div>
                    <div>
                        <label>Hide Property Address for non-register User. </label>
                        <select id="hide_property_address" name="Listing[hide_property_address]">
                            {foreach from=$arrYesNo key=key item=item}
                                <option value="{$key}" {if $key == $arrListingConfig['hide_property_address']}selected {elseif $arrListingConfig['hide_property_address'] == '' && $key == 'No'}selected{/if}>{$item}</option>
                            {/foreach}
                        </select>
                    </div>
                    <div>
                ...
                <div class="span8">
                   ...
                   {* <div>
                    ...
                    <div>
                        ...
                        <textarea id="page_metakeyword_listing_detail" name="PageConfig[page-metakeyword-detail]" class="input-xxlarge">{$arrPageConfig['page-metakeyword-detail']['Format']}</textarea>
            ...
            <div class="row-fluid">
                ...
                <div class="span8">
                    ...
                    <div>
                        ...
                        <input type="text" id="hover_color" autocomplete="off" name="OtherConfig[hover_color]" value="{if isset($arrOtherConfig['hover_color']) && $arrOtherConfig['hover_color'] != ''}{$arrOtherConfig['hover_color']}{else}#ffffff{/if}" class="input-xlarge"/>
                        <label class="error" for="hover_color">Enter Button Hover color</label>
                    </div>
                   <br>
                    <div>
                        <label>Hover Text Color</label>
                        <input type="text" id="hover_text_color" autocomplete="off" name="OtherConfig[hover_text_color]" value="{if isset($arrOtherConfig['hover_text_color']) && $arrOtherConfig['hover_text_color'] != ''}{$arrOtherConfig['hover_text_color']}{else}#000000{/if}" class="input-xlarge"/>
                        <label class="error" for="hover_text_color">Enter Button Hover Text color</label>
                    </div>
        ...
        <div id="tab6" class="tab-pane well wellmedpadd">
            ...
            <div class="row-fluid">
                <div class="mb-2">
                    <label><b>Enable Redis:</b></label>
                    <div class="span8" style="display: inline-flex;">
                        {*<select id="redis_enable" name="redis_enable">
                            {foreach from=$arrYesNo key=key item=item}
                                <option value="{$key}" {if $redisArr == '' && $key == 'Yes'}selected {elseif $key == $redisArr}selected{/if}>{$item}</option>
                            {/foreach}
                        </select>*}
                        <input type="radio" id="yes" name="redis_enable" value="Yes" {if $redisArr == '' || $redisArr == 'Yes'}checked{/if}>
                        <label for="Yes">&nbsp; Yes</label><br>
                        &nbsp;&nbsp;
                        <input type="radio" id="no" name="redis_enable" value="No"{if $redisArr != '' && $redisArr == 'No'}checked{/if}>
            ...
            <div class="row-fluid">
                ...
                <div class="span8">
                    ...
                    <div>
                        ...
                        <p> <b>Note:</b> Please enter only one option from center, left and right to align search bar title. </p>
                    </div>
            ...
            <div class="row-fluid">
                <div>
                    ...
                    <textarea id="google_conv_code" name="OtherConfig[google_conv_code]" placeholder="Google Conversion Code" rows="7" class="input-xxlarge">{$arrOtherConfig['google_conv_code']}</textarea>
                   ...
                   {* {if isset($AgentSystemName) && $AgentSystemName == {constant('Constants::ACTRIS')}}
                        ...
                        <div class="multi-opt cols3 multi-options-container">
                            {foreach name=ptype from=$arrMeta['SubTypeActris'] key=skey item=sitem}
                                <label><input type="checkbox" name="OtherConfig[quick_ptype][]" {if $skey|in_array:$arrOtherConfig['quick_ptype'] || $skey == $arrOtherConfig['quick_ptype']}checked="checked"{/if} id="{$skey}" value="{$skey}" />&nbsp;{if $sitem == 'Lease'}Rental{else}{$sitem}{/if}</label>
                            {/foreach}
                        </div>
                    {else}
                        <label><b> MIAMI/BEACHES </b></label>*}
                        <div class="multi-opt cols3 multi-options-container">
                            {foreach name=ptype from=$arrMeta['SubType'] key=skey item=sitem}
                                <label><input type="checkbox" name="OtherConfig[quick_ptype][]" {if (isset($arrOtherConfig['quick_ptype'])) && ($skey|in_array:$arrOtherConfig['quick_ptype'] || $skey == $arrOtherConfig['quick_ptype'])}checked="checked"{/if} id="{$skey}" value="{$skey}" />&nbsp;{if $sitem == 'Lease'}Rental{else}{$sitem}{/if}</label>
                            {/foreach}
                        </div>
                    {*{/if}*}
					<div>
			...
			<div class="row-fluid">
				...
				<div class="span8">
                   ...
                   {* {if isset($AgentSystemName) && $AgentSystemName == {constant('Constants::ACTRIS')}}
                        ...
                        <div class="multi-opt cols3 multi-options-container">
                            {foreach name=ptype from=$arrMeta['SubTypeActris'] key=skey item=sitem}
                                <label><input type="checkbox" name="OtherConfig[quick_ptype2][]" {if $skey|in_array:$arrOtherConfig['quick_ptype2'] || $skey == $arrOtherConfig['quick_ptype2']}checked="checked"{/if} id="{$skey}" value="{$skey}" />&nbsp;{if $sitem == 'Lease'}Rental{else}{$sitem}{/if}</label>
                            {/foreach}
                        </div>
                    {else}
                        <label><b> MIAMI/BEACHES </b></label>*}
                        <div class="multi-opt cols3 multi-options-container">
                            {foreach name=ptype from=$arrMeta['SubType'] key=skey item=sitem}
                                <label><input type="checkbox" name="OtherConfig[quick_ptype2][]" {if (isset($arrOtherConfig['quick_ptype2'])) && ($skey|in_array:$arrOtherConfig['quick_ptype2'] || $skey == $arrOtherConfig['quick_ptype2'])}checked="checked"{/if} id="{$skey}" value="{$skey}" />&nbsp;{if $sitem == 'Lease'}Rental{else}{$sitem}{/if}</label>
                            {/foreach}
                        </div>
                   {* {/if}*}
					<div>
            ...
            <div class="row-fluid">
                ...
                <div class="span8">
                    ...
                    <div>
                        <label> Text Color</label>
                        <input type="text" id="quick_text_color" name="OtherConfig[style9_text_color]" value="{$arrOtherConfig['style9_text_color']|default:'#000'}" class="input-xxlarge"/>
                        <p> <b>Note:</b> Please enter color code. </p>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="style9_bg_color" name="OtherConfig[style9_bg_color]" value="{$arrOtherConfig['style9_bg_color']|default:'#ffffff00'}" class="input-xxlarge"/>
                        <p> <b>Note:</b> Please enter color code. </p>
                    </div>

                    <div>
                        <label> Border Hover Color</label>
                        <input type="text" id="style9_bdr_hvr_color" name="OtherConfig[style9_bdr_hvr_color]" value="{$arrOtherConfig['style9_bdr_hvr_color']|default:'#000'}" class="input-xxlarge"/>
                        <p> <b>Note:</b> Please enter color code. </p>
                    </div>

                    <div>
                        <label> Background Hover Color</label>
                        <input type="text" id="style9_bg_hvr_color" name="OtherConfig[style9_bg_hvr_color]" value="{$arrOtherConfig['style9_bg_hvr_color']|default:'#ffff'}" class="input-xxlarge"/>
                        <p> <b>Note:</b> Please enter color code. </p>
                    </div>
                </div>
            </div>
            ...
            <div class="row-fluid">
                ...
                <div class="span8">
                    ...
                    {if $arrShortCode && count($arrShortCode) > 0}
                        ...
                        {/foreach}
                        <div>
                            <b>[predefine-search-result agent=1 pid=]</b> :
                            <br/>
                            You can use it with any post/page to display map on left side with priced markers and properties in grid format on right side based on predefined search results which is created by you.<br/>
                            You can see the predefined short code for "created predefined" in Plugin Back-End "My Predefined Searches" module.<br/>
                            If you want to display predefine search result in grid format then you can pass “isgrid” parameters in this short code. For Example: <b>[predefine-search-result ag=1 pid=1 isgrid=true]</b>
                ...
                <div class="span8 border">
                    <hr/>
                    <h4>Save search alert cron jobs setup information</h4>
                    <b>Note:</b> Please setup below listed cron jobs on your server.
                    <table class="wp-list-table table table-striped responsive-table" width="100%">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td width="5%">*</td>
                                <td width="5%">*</td>
                                <td width="20%">{$cronPath}send_search_alert.php</td>
                            </tr>
                            <tr>
                                <td width="5%">7,22,37,52</td>
                                <td width="5%">*</td>
                                <td width="5%">*</td>
                                <td width="5%">*</td>
                                <td width="5%">*</td>
                                <td width="20%">{$cronPath}send_lead_mail.php</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
