...
class AdminAjaxRequest
{
    ...
    public function requestHandler()
    {
        ...
        if ($strModule == 'pred-listing') {
            ...
            $ret = AgentPredefined::getInstance()->requestHandler($isAjaxRequest, $strModule);
        ...
        elseif ($strModule == 'listing-count'){
        ...
        elseif ($strModule == 'master-prelist-pagination')
        {
            include_once $arrPhysicalPath['UserBase'] . 'MasterPredefined.php';
            $ret = MasterPredefined::getInstance()->requestHandler($isAjaxRequest, $strModule);
        }
        elseif ($strModule == 'pre-search')
        {
	        include_once $arrPhysicalPath['UserBase'] . 'AgentPredefined.php';
	        $ret = AgentPredefined::getInstance()->requestHandler($isAjaxRequest, $strModule);
        }elseif ($strModule == 'master-pre-search')
        {
	        include_once $arrPhysicalPath['UserBase'] . 'MasterPredefined.php';
	        $ret = MasterPredefined::getInstance()->requestHandler($isAjaxRequest, $strModule);
        }
