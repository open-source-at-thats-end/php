    ...
    infoHtml += '<div id="'+obj.Key+'" class="listing-box position-relative" data-ref-id="'+obj.Key+'">';
        ...
        if (hideAddress != undefined && hideAddress == 'Yes') {
            if (isUserLoggedIn != undefined && isUserLoggedIn != '' && isUserLoggedIn == true) {
                // popup = '<a href="' + Site_Url + obj.SFUrl + '" class="te-property-card te-font-family te-style-11 d-block te-property-gradient position-relative overflow-hidden">' ;
                listing_address = '<div class="te-property-title text-truncate te-font-size-11 te-font-weight-400">'+ obj.Address +'</div>';
            }else{
                // popup = '<a href="JavaScript:void(0);" class="popup-modal-sm te-property-card te-font-family te-style-11 d-block te-property-gradient position-relative overflow-hidden" aria-label="button" data-toggle="modal" data-target="MemberLogin" data-url="' + Site_Url + memberDetail + '/?action=member-login&mlsNum=' + obj.Key + '">';
        ...
        if (agent_Profile_Id != undefined && agent_Profile_Id != '' ) {
            infoHtml += '<a href="' + Site_Url + obj.SFUrl + "aid-" + agent_Profile_Id + '/" class="te-property-card te-font-family te-style-11 d-block te-property-gradient position-relative overflow-hidden">';
        }
        else{
        ...
        infoHtml +=     '<img src="'+ url +'" class="te-property-fig te-property-image position-absolute lzl-" alt="'+jQuery.trim(obj.AddressSort)+'-1">' +
                        ...
                        '<div class="te-smooth-gradient te-property-details te-animate text-white position-absolute te-z-index-99 '+sclass+' te-p-5">' +
                            '<div class="-te-gradient te-trans  te-property-details te-animate px-3 py-2- py-3 text-white position-absolute te-z-index-99 crypto-data">';
        if(AgentCryptoValue == true) {
            if (obj.Type == "ResidentialLease") {
                ...
                infoHtml += '<li class="pr-1 text-center"><img class="d-block mx-auto mb-1 te-font-weight-500 " src="' + Site_Url + 'wp-content/plugins/CustomWpPlugin/upload/pictures/bitcoin.png" alt="bitcoin"> APPROX <span class="d-block">' + numberFormat(Math.round(obj.Price / bitcoin)) + '</span></li>';

                infoHtml += '<li class="px-1 text-center"><img class="d-block mx-auto mb-1 te-font-weight-500 " src="' + Site_Url + 'wp-content/plugins/CustomWpPlugin/upload/pictures/etherium.png" alt="etherium"> APPROX <span class="d-block">' + numberFormat(Math.round(obj.Price / etherium)) + '</span></li>';
       ...
       /* infoHtml += '<div class="te-property-title  text-truncate te-font-size-14">'+ obj.subdiv +'</div>';*/
...

        {
            ...
            infoHtml += '<div class=" pl-2 d-inline te-property-details-price-change text-wrap py-1 te-font-size-15 te-font-weight-600 ' + text + '">'+ arrow + currency + numberFormat(price)+'</div>';
