...
function getMainBoxHtml(obj, fav) {
    ...
    if (hideAddress != undefined && hideAddress == 'Yes') {
        if (isUserLoggedIn != undefined && isUserLoggedIn != '' && isUserLoggedIn == true) {
            // popup = '<a href="'+ Site_Url+obj.SFUrl+'" class="'+h_class+' d-block te-property-gradient te-font-family te-style-5 position-relative overflow-hidden">';
            listing_address = '<div class="style4-title text-truncate te-font-size-14 pb-1- te-font-weight-500">'+ obj.Address +'</div>';
        }else{
            // popup = '<a href="JavaScript:void(0);" class="popup-modal-sm '+h_class+' d-block te-property-gradient te-font-family te-style-5 position-relative overflow-hidden" aria-label="button" data-toggle="modal" data-target="MemberLogin" data-url="' + Site_Url + memberDetail + '/?action=member-login&mlsNum=' + obj.Key + '">';
            listing_address = '<div class="style4-title text-truncate te-font-size-14 pb-1- te-font-weight-500"><span href="JavaScript:void(0);" class="popup-modal-sm" aria-label="button" data-toggle="modal" data-target="MemberLogin" data-url="' + Site_Url + memberDetail + '/?action=member-login&mlsNum=' + obj.Key + '"> Register For Details </span></div>';
        }
    }else{
        listing_address = '<div class="style4-title text-truncate te-font-size-14 pb-1- te-font-weight-500">'+ obj.Address +'</div>';
        // popup = '<a href="'+ Site_Url+obj.SFUrl+'" class="'+h_class+' d-block te-property-gradient te-font-family te-style-5 position-relative overflow-hidden">';
    ...
    if (agent_Profile_Id != undefined && agent_Profile_Id != '' ) {
        infoHtml += '<a href="' + Site_Url + obj.SFUrl + "aid-" + agent_Profile_Id + '/" class="' + h_class + ' d-block te-property-gradient te-font-family te-style-5 position-relative overflow-hidden">';
    }
    else{
    ...
    if(AgentCryptoValue == true) {
        ...
        if (obj.Type == "ResidentialLease") {
            ...
            infoHtml += '<li class="pr-1 text-center"><img class="d-block mx-auto mb-1" src="' + Site_Url + 'wp-content/plugins/CustomWpPlugin/upload/pictures/bitcoin.png" alt="bitcoin"> APPROX <span class="d-block">' + numberFormat(Math.round(obj.Price / bitcoin)) + '</span></li>';

            infoHtml += '<li class="px-1 text-center"><img class="d-block mx-auto mb-1" src="' + Site_Url + 'wp-content/plugins/CustomWpPlugin/upload/pictures/etherium.png" alt="etherium"> APPROX <span class="d-block">' + numberFormat(Math.round(obj.Price / etherium)) + '</span></li>';
    ...
    {
        infoHtml += '<div class="style4-features te-font-size-14 te-font-weight-500-">' + numberFormat(obj.Sqft) + ' Sq Ft</div>';
    }
    //infoHtml += '<div class="style4-features te-font-size-14 te-font-weight-500">'+obj.Bed+' Beds <span>|</span> '+obj.BathsFull+' Baths <span>|</span> '+numberFormat(obj.Sqft)+' Sq Ft</div>';
