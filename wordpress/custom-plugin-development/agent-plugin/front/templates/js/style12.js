...
//console.log(data);
    ...
    infoHtml += '<span data-id='+pid+' class="arrow-prev angle-right- icon-2x bg-transparent te-font-weight-500 '+cust_color+' pr-2 ">Previous </span><span class=" '+cust_color+' te-font-weight-500">|</span><span data-id='+pid+' class="arrow-next angle-right- icon-2x bg-transparent '+cust_color+' te-font-weight-500 mr-md-4 pl-2"> Next</span>';
    infoHtml += '</div>';
    infoHtml += '</div>';
    ...
    jQuery.each(data, function (index, obj) {
        ...
        if (hideAddress != undefined && hideAddress == 'Yes') {
            if (isUserLoggedIn != undefined && isUserLoggedIn != '' && isUserLoggedIn == true) {
                infoHtml += '<h5 class="text-white pb-0 text-capitalize te-font-weight-500">'+obj.Address2+'</h5></div>';
                // var view_detail = '<div class="property-info"><div class="text-right pb-sm-3 pb-md-0"><a href="'+ Site_Url+obj.SFUrl+'" class="btn btn-style1 text-white text-uppercase mt-3 px-sm-5 py-sm-2- py-lg-3- cust-padding '+ details_box +'"><span class="te-font-size-11">VIEW DETAILS</span></a></div></div>';
            }else{
                infoHtml += '<span href="JavaScript:void(0);" class="popup-modal-sm text-white pb-0 text-capitalize te-font-weight-500" aria-label="button" data-toggle="modal" data-target="MemberLogin" data-url="' + Site_Url + memberDetail + '/?action=member-login&mlsNum=' + obj.Key + '">';
                infoHtml += 'Register For Details';
                infoHtml += '</span>';
                infoHtml += '</div>';
                // var view_detail = '<div class="property-info"><div class="text-right pb-sm-3 pb-md-0"><a href="JavaScript:void(0);" class="popup-modal-sm btn btn-style1 text-white text-uppercase mt-3 px-sm-5 py-sm-2- py-lg-3- cust-padding '+ details_box +'" aria-label="button" data-toggle="modal" data-target="MemberLogin" data-url="' + Site_Url + memberDetail + '/?action=member-login&mlsNum=' + obj.Key + '"><span class="te-font-size-11">VIEW DETAILS</span></a></div></div>';
            }
        }else{
            infoHtml += '<h5 class="text-white pb-0 text-capitalize te-font-weight-500">'+obj.Address2+'</h5></div>';
            // var view_detail = '<div class="property-info"><div class="text-right pb-sm-3 pb-md-0"><a href="'+ Site_Url+obj.SFUrl+'" class="btn btn-style1 text-white text-uppercase mt-3 px-sm-5 py-sm-2- py-lg-3- cust-padding '+ details_box +'"><span class="te-font-size-11">VIEW DETAILS</span></a></div></div>';
        }
        // infoHtml += '<div class="pb-1"><h5 class="text-white pb-0 text-capitalize te-font-weight-500">'+obj.Address2+'</h5></div>';
        infoHtml += '<div class="te-property-details-features te-font-size-12 pb-2- text-capitalize te-font-weight-500">'+obj.CityName+', '+ obj.State +' '+obj.ZipCode+'</div>';
                        ...
                        if (agent_Profile_Id != undefined && agent_Profile_Id != '' ) {
                            infoHtml += '<div class="property-info"><div class="text-right pb-sm-3 pb-md-0"><a href="' + Site_Url + obj.SFUrl + "aid-" + agent_Profile_Id + '/" class="btn btn-style1 text-white text-uppercase mt-3 px-sm-5 py-sm-2- py-lg-3- cust-padding ' + details_box + '"><span class="te-font-size-11">VIEW DETAILS</span></a></div></div>';
                        }
                        else{
