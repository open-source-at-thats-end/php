...
function getMainBoxHtml(obj, fav) {
    ...
    if (hideAddress != undefined && hideAddress == 'Yes') {
        if (isUserLoggedIn != undefined && isUserLoggedIn != '' && isUserLoggedIn == true) {
            // popup = '<a href="'+ Site_Url+obj.SFUrl+'" class="te-property-card te-font-family d-block te-property-gradient position-relative overflow-hidden dfelt-style te-style-4 te-style-default">';
            listing_address = '<div class="te-property-title text-truncate te-font-size-11 te-font-weight-400">'+ obj.Address2 +'</div>';
        }else{
            // popup = '<a href="JavaScript:void(0);" class="popup-modal-sm te-property-card te-font-family d-block te-property-gradient position-relative overflow-hidden dfelt-style te-style-4 te-style-default" aria-label="button" data-toggle="modal" data-target="MemberLogin" data-url="' + Site_Url + memberDetail + '/?action=member-login&mlsNum=' + obj.Key + '">';
            listing_address = '<div class="te-property-title text-truncate te-font-size-11 te-font-weight-400"><span href="JavaScript:void(0);" class="popup-modal-sm" aria-label="button" data-toggle="modal" data-target="MemberLogin" data-url="' + Site_Url + memberDetail + '/?action=member-login&mlsNum=' + obj.Key + '"> Register For Details </span></div>';
        }
    }else{
        listing_address = '<div class="te-property-title text-truncate te-font-size-11 te-font-weight-400">'+ obj.Address2 +'</div>';
        // popup = '<a href="'+ Site_Url+obj.SFUrl+'" class="te-property-card te-font-family d-block te-property-gradient position-relative overflow-hidden dfelt-style te-style-4 te-style-default">';
    ...
    if (agent_Profile_Id != undefined && agent_Profile_Id != '' ) {
        infoHtml += '<a href="' + Site_Url + obj.SFUrl + "aid-" + agent_Profile_Id + '/" class="te-property-card te-font-family d-block te-property-gradient position-relative overflow-hidden dfelt-style te-style-4 te-style-default">';
    }
    else{
        infoHtml += '<a href="' + Site_Url + obj.SFUrl + '" class="te-property-card te-font-family d-block te-property-gradient position-relative overflow-hidden dfelt-style te-style-4 te-style-default">';
    }
    ...
    infoHtml += '<div class="te-smooth-gradient te-property-details te-animate text-white position-absolute te-z-index-99 '+sclass+' te-p-5">' +
        '<div class="-te-gradient te-trans  te-property-details te-animate px-3 py-3 text-white position-absolute te-z-index-99 crypto-data">';
    ...
    if(AgentCryptoValue == true) {
        ...
        if (obj.Type == "ResidentialLease") {
            ...
            infoHtml += '<li class="pr-1 text-center"><img class="d-block mx-auto mb-1 te-font-weight-400 " src="' + Site_Url + 'wp-content/plugins/CustomWpPlugin/upload/pictures/bitcoin.png" alt="bitcoin"> APPROX <span class="d-block">' + numberFormat(Math.round(obj.Price / bitcoin)) + '</span></li>';

            infoHtml += '<li class="px-1 text-center"><img class="d-block mx-auto mb-1 te-font-weight-400 " src="' + Site_Url + 'wp-content/plugins/CustomWpPlugin/upload/pictures/etherium.png" alt="etherium"> APPROX <span class="d-block">' + numberFormat(Math.round(obj.Price / etherium)) + '</span></li>';
    ...
    {
        infoHtml += '<div class="te-property-details-price d-inline te-font-weight-600 te-font-size-15 ">'+ currency + numberFormat(obj.SoldPrice)+'</div>';
    }
    else{
        infoHtml += '<div class="te-property-details-price d-inline te-font-weight-600 te-font-size-15 ">'+ currency + numberFormat(obj.Price)+'</div>';
    }

    infoHtml += listing_address;

    // infoHtml += '<div class="te-property-title text-truncate te-font-size-11 te-font-weight-400">'+ obj.Address2 +'</div>';
...

    {
        ...
        infoHtml += '<div class=" pl-2 d-inline te-property-details-price-change text-wrap pb-2 te-font-weight-600 te-font-size-15 ' + text + '">'+ arrow + currency + numberFormat(price)+'</div>';


    }
    infoHtml += '<div class="te-property-details-features te-font-size-10"><ul class="d-flex text-center te-font-weight-400 justify-content-end p-0 te-font-size-11 pb-2 '+ cus_pt +'">';
    ...
    infoHtml += '<li class="pl-2 pl-lg-2 pl-xl-2 text-center"> '+numberFormat(obj.Sqft)+' </br> Sq. Ft.</li></ul></div>';
    infoHtml += '</div>';
...
function getMainBoxHtmlInfo(obj, fav) {
    ...
    // infoHtml += '<div id="' + obj.Key + '" class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 p-0" data-ref-id="' + obj.Key + '">';
    infoHtml += '<a href="'+ Site_Url+obj.SFUrl+'" class="te-property-card te-style-4 d-block te-property-gradient position-relative overflow-hidden dflt-style te-style-default">';
    if (obj.Url_Support == 'Yes' && obj.TotalPhotos > 0) {
        if (typeof obj.Pic.large != 'undefined' && obj.Pic.large.url != null && obj.Pic.large.url != '') {
            infoHtml += '<img data-lzl="'+ obj.Pic.large.url +'" class="te-property-fig te-property-image position-absolute lzl" alt="'+jQuery.trim(obj.AddressSort)+'-1">';
        }
    ...
    {
        infoHtml += '<div class="te-property-details-features te-font-size-12">'+numberFormat(obj.Sqft)+' Sq Ft</div>';
    }
    /*infoHtml += '<div class="te-property-details-features te-font-size-12">'+obj.Bed+' Beds <span>|</span> '+obj.BathsFull+' Baths <span>|</span> '+numberFormat(obj.Sqft)+' Sq Ft</div>';*/
