...
function getListingBoxHtml(data){
   ...
   infoHtml += '<div class="properties-slider te-font-family position-relative '+ style_1_7 +'">';
    ...
    /*infoHtml += '</div>' +*/
          ...
          if(AgentCryptoValue == true) {
              ...
              if (obj.Type == "ResidentialLease") {
                  ...
                  infoHtml += '<div class="crypto-data pb-1"><ul class="d-flex p-0 te-font-size-10 justify-content-between- py-1 py-xl-2">' +
                      '<li class="pr-1 text-center"><img class="d-block mx-auto mb-1" src="' + Site_Url + 'wp-content/plugins/CustomWpPlugin/upload/pictures/bitcoin.png" alt="bitcoin"> APPROX <strong class="d-block">' + numberFormat(Math.round(obj.Price / bitcoin)) + '</strong></li>' +
                      '<li class="px-1 text-center"><img class="d-block mx-auto mb-1" src="' + Site_Url + 'wp-content/plugins/CustomWpPlugin/upload/pictures/etherium.png" alt="etherium"> APPROX <strong class="d-block">' + numberFormat(Math.round(obj.Price / etherium)) + '</strong></li>' +
          ...
          infoHtml +=   '<div class="pb-1-">';
          if (hideAddress != undefined && hideAddress == 'Yes') {
              if (isUserLoggedIn != undefined && isUserLoggedIn != '' && isUserLoggedIn == true) {
                  infoHtml +=   '<h3 class="text-white pb-2 pb-0 text-uppercase te-font-size-24 te-address">'+obj.Address2+'</h3>';
                  // var view_detail = '<div class="property-info"><div class="text-right pb-sm-3 pb-md-0"><a href="'+ Site_Url+obj.SFUrl+'" class="btn btn-style1 text-white text-uppercase mt-3 px-sm-5 py-sm-2- py-lg-3- cust-padding '+ details_box +'"><span class="te-font-size-12">Details</span></a></div></div>';
              }else{
                  infoHtml += '<a href="JavaScript:void(0);" style="z-index: 9999; position: relative;" class="popup-modal-sm" aria-label="button" data-toggle="modal" data-target="MemberLogin" data-url="' + Site_Url + memberDetail + '/?action=member-login&mlsNum=' + obj.Key + '">';
                  infoHtml += '<h3 class="text-white pb-2 pb-0 text-uppercase te-font-size-24 te-address">Register For Details</h3>';
                  infoHtml += '</a>';
                  // var view_detail = '<div class="property-info"><div class="text-right pb-sm-3 pb-md-0"><a href="JavaScript:void(0);" class="popup-modal-sm btn btn-style1 text-white text-uppercase mt-3 px-sm-5 py-sm-2- py-lg-3- cust-padding '+ details_box +'" aria-label="button" data-toggle="modal" data-target="MemberLogin" data-url="' + Site_Url + memberDetail + '/?action=member-login&mlsNum=' + obj.Key + '"><span class="te-font-size-12">Details</span></a></div></div>';
          ...
          infoHtml += '</div>';
          // infoHtml +=   '<div class="pb-1-"><h3 class="text-white pb-2 pb-0 text-uppercase te-font-size-24 te-address">'+obj.Address2+'</h3></div>';
          ...
          if (jQuery.inArray(obj.Type, JSON.parse(arrPType)) < 0 && jQuery.inArray(obj.SubType, JSON.parse(arrSType)) < 0) {
              LDetails = ' / ' + obj.Bed + ' Beds / ' + obj.BathsFull + ' Baths / ' + numberFormat(obj.Sqft) + ' Sq Ft';
          } else {
              LDetails = ' / ' + numberFormat(obj.Sqft) + ' Sq Ft';
          }

          if(jQuery.inArray(obj.Type, JSON.parse(arrPType)) < 0 && jQuery.inArray(obj.SubType, JSON.parse(arrSType)) < 0)
              var info = '&nbsp;<i>'+obj.Bed+' Beds  </i>&nbsp;|&nbsp; <i>'+obj.BathsFull+' Baths  </i>&nbsp;|';
          else
              var info = '';

          if(obj.status == 'Closed')
          {
              infoHtml += '<div class="te-property-details-price- d-md-inline pb-sm-3 te-font-size-17 w-50 w-md-100 '+ prop_details +'">'+ currency + numberFormat(obj.SoldPrice) +'  &nbsp;|' + info + ' &nbsp; '+numberFormat(obj.Sqft)+' Sq Ft</div>';
          }
          else{
              infoHtml += '<div class="te-property-details-price- d-md-inline te-font-size-17 pb-sm-3 w-50 w-md-100 '+ prop_details +'"><i>'+ currency + numberFormat(obj.Price) +' </i>&nbsp; |' + info + ' &nbsp; <i>'+numberFormat(obj.Sqft)+' Sq Ft</i></div>';
          }
          infoHtml += price_diff;
          if (agent_Profile_Id != undefined && agent_Profile_Id != '' ) {
              infoHtml += '<div class="property-info"><div class="text-right pb-sm-3 pb-md-0"><a href="' + Site_Url + obj.SFUrl + "aid-" + agent_Profile_Id + '/" class="btn btn-style1 text-white text-uppercase mt-3 px-sm-5 py-sm-2- py-lg-3- cust-padding ' + details_box + '"><span class="te-font-size-12">Details</span></a></div></div>';
          }
          else {
