<div class="{if isset($style) && $style == 6}qstyle6 mx-0 w-100{/if} quick-container {if isset($style) && $style == 5}quick-style{/if} {if isset($style) && ($style == 2 || $style == 3 || $style == 4 || $style == 5 || $style == 6)}h-container{/if} te-font-family" style="{if isset($style) && ($style == 2 || $style == 3 || $style == 4 || $style == 5)}height:100vh{/if} ">
	    ...
	    <div class="style2-content style3-content quick-content mw-100">
            <form action="{$formAction}" method="post" class="w-100">
				<div class="text-center">
					{if (isset($isTitle) && $isTitle !== 'false' && $isTitle !== false) && (isset($OtherConfig['quick3_title']) && $OtherConfig['quick3_title'] != '')}
					...
					<span class=" position-relative d-inline-block">
						...
						<select name="stype" class="quick3-style f-27 f-select lpt-quick-color-">
							...
							{foreach name=ptype from=$OtherConfig.quick_ptype2 key=skey item=sitem}
								{*{if isset($AgentSystemName) && $AgentSystemName == {constant('Constants::ACTRIS')}}
									<option value="{$sitem}">{$arrMeta['SubTypeActirs'][$sitem]}</option>
								{else}*}
				...
				<form class="form mw-100 " id="frmsearch" role="form" action="{$formAction}" method="post">
					<div class="btn-toolbar justify-content-center" role="toolbar" aria-label="Toolbar with button groups">
						<div class="btn-group w-100- te-search-btn-group {*{if isset($darkmode) && $darkmode == 'true'}bg-black{else}bg-white{/if}*}" role="group" aria-label="First group" style="background-color: {$bgcolor}">
							<div class="status-div">
								<span class=" position-relative d-inline-block status"><i class="fa fa-chevron-down f-arrow fa-sm" aria-hidden="true"></i>
								<select class="{if isset($darkmode) && $darkmode == 'true'}bg-black- text-white{/if} styl6 font-weight-bold" name="status">
								<option value="active">BUY</option>
								<option value="rental">RENT</option>
								</select></span>
							</div>
							...
							<button id="search-box" type="submit" aria-label="button" class="btn px-lg-4 px-3 {if cw::$screen == 'XS'}pr-4{/if} shadow-none rounded-0 pad-right {if isset($darkmode) && $darkmode == 'true'}text-white{/if} font-weight-bold" style="background-color: {$bgcolor}">
	...
	{else}
		<div class="quick-content">
			<div class="float-left pt-4">
				<h2><strong class="head-title quick-head-title quick_text_color quick-style quick_font_family pl-0">QUICK</strong>
					<strong class="head-sub-title quick-sub-title quick_text_color quick-style quick_font_family mt-3 pl-0">SEARCH</strong>
				</h2>
			</div>
				...
				<form action="{$formAction}" method="post">
					<div class="quick-row">
						<div class="quick quick-w-100 quick_text_color arrow-clr">
							<select name="stype" class="quick-ptype quick_font_size quick_font_family">
								<option value="">PROPERTY TYPE</option>
						...
						<div class="quick quick-w-100 quick_text_color arrow-clr">
							<select name="city" class="quick_font_size quick_font_family">
								<option value="">SELECT A CITY</option>
						...
						<div class="quick quick_text_color arrow-clr">
							<select name="minprice" class="quick_font_size quick_font_family">
								{foreach name=minprice from=$arrPriceRange key=mkey item=mitem}
									<option value="{$mkey}">{$mitem}</option>
								{/foreach}
						...
						<div class="quick quick_text_color arrow-clr">
							<select name="maxprice" class="quick_font_size quick_font_family">
					...
					<div class="quick-row mt-3">
						<div class="quick quick_text_color arrow-clr">
							<select name="minbed" class="minimal quick_font_size quick_font_family">
								<option value="">BEDS</option>
								{foreach name=minbed from=$arrBathRange key=bedkey item=beditem}
									<option value="{$bedkey}">{$beditem}</option>
								{/foreach}
						...
						<div class="quick quick_text_color arrow-clr">
							<select name="minbath " class="quick_font_size quick_font_family">
								<option value="">BATHS</option>
								{foreach name=minbath from=$arrBathRange key=bathkey item=bathitem}
									<option value="{$bathkey}">{$bathitem}</option>
								{/foreach}
						...
						<div class="quick-w-100 quick-btn">
							<div class="quick ">
								<input type="submit" class="quick_text_color quick_btn_color quick_font_size quick_font_family" value="SEARCH"/>
							</div>
							<div class="quick quick-m-t-4">
								<a href="{$formAction}" class="quick_text_color quick_btn_color quick_font_size quick_font_family"><span>Advanced</span></a>
							</div>
						</div>
					</div>
					<input type="hidden" name="status" value="active" class="status"> 
