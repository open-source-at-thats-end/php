<div class="modal-header">
	{if isset($arrConfig['Site_Agent']['agent_signin_text']) && $arrConfig['Site_Agent']['agent_signin_text'] != ''}
		{assign var="HeaderText" value="{$arrConfig['Site_Agent']['agent_signin_text']}"}
	{else}
		{assign var="HeaderText" value="Sign In"}
	{/if}
	<h5 class="modal-title txt-heading heading_txt_color" id="exampleModalLabel">{$HeaderText}</h5>
...
<div class="modal-body pb-0 mt-3">
	<form for="form" id="frmLogin">
		...
		<input type="hidden" name="isag" id="isag" value="{$smarty.request.isag|default:''}"/>
	...
	<div class="form-group mt-3 text-center">
		<p class="te-font-size-14 p-0">Trouble Logging In? <u><a class="text-main text-decoration-none popup-modal-sm link-color" href="JavaScript:void(0);"  data-toggle="modal" data-target="MemberRegister" data-url="{$Site_Url}{Constants::TYPE_MEMBER_DETAIL}/?action=member-register{if isset($ReqType)}&ReqType={$ReqType}{/if}{if isset($mlsNum)}&mlsNum={$mlsNum}{/if}" data-dismiss="modal">Reset Password</a></u></p>
	</div>
