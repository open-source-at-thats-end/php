...
if( !class_exists('FrontModuleDispatcher')) {
	...
	class FrontModuleDispatcher {
		...
		private function init(){
			global $wp_query, $arrPhysicalPath, $arrVirtualPath, $objTmpl, $arrConfig;
			...
			if( !$this->initialized && $postsCount == 0 ){
				...
				if( $type = get_query_var(Constants::TYPE_URL_VAR) ) {
					...
					{
						wp_enqueue_style( 'p-market-report', $arrVirtualPath['TemplateCss']. 'market-reports.css',array(),Constants::CSS_JS_VERSION);
                        wp_enqueue_style('ore-color-style');
					...
					{
						wp_enqueue_style( 'p-user-dashboard', $arrVirtualPath['TemplateCss']. 'user-dashboard.css',array(),Constants::CSS_JS_VERSION);
						//wp_enqueue_style( 'p-search-result', $arrVirtualPath['Libs']. 'front/css/search-results.css',array(),Constants::CSS_JS_VERSION);
						wp_enqueue_style( 'p-search-result', $arrVirtualPath['TemplateCss']. 'search-results.css',array(),Constants::CSS_JS_VERSION);
                        //wp_enqueue_style('srchresult');
                        wp_enqueue_style( 'p-mls-property', $arrVirtualPath['TemplateCss']. 'mls-properties-embedding.css',array(),Constants::CSS_JS_VERSION);
