...
global $arrPhysicalPath, $arrVirtualPath, $arrConfig, $objTmpl, $objVWDb, $objAjaxResp, $recContent, $objAPI,$db;
...
include_once $arrPhysicalPath['DBAccess']. 'DAO.php';
include_once $arrPhysicalPath['DBAccess']. 'IDXAPI.php';


include($arrPhysicalPath['Base']. '/includes/static_data.php');
...
$db = $objDB;
/*$data = Utility::getInstance()->get_custom_options();

//echo stripslashes($data['OtherConfig']['google_conv_code']);die;*/
# Read Config
$arrConfig = get_option(Constants::OPTIONS);
//$arrConfig = get_option('lpt_config');
...
if(strpos($_SERVER['HTTP_HOST'], 'project') !== false)
{
    $arrConfig['idxapi_url'] = 'http://brokeridx-api.project:'.$_SERVER['SERVER_PORT'].'/apis/DataFetcher.php';
    $arrConfig['autosugg_url'] = 'http://brokeridx-api.project:'.$_SERVER['SERVER_PORT'].'/autosuggest.php';
    $arrConfig['upload_url'] = 'http://brokeridx-api.project:'.$_SERVER['SERVER_PORT'].'/upload/';
...
elseif(strpos($_SERVER['HTTP_HOST'], 'thatsend') !== false)
{
    ...
    $arrConfig['idxapi_url'] = $_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST'].'/API/apis/DataFetcher.php';
    $arrConfig['autosugg_url'] = $_SERVER['HTTP_X_FORWARDED_PROTO'].'://'.$_SERVER['HTTP_HOST'].'/API/autosuggest.php';
    $arrConfig['upload_url'] = $_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST'].'/API/upload/';
}
...
if(!defined("IN_CRON") && !defined("IN_ASSETS"))
{
    ...
    add_action( 'upgrader_process_complete', array(Installer::getInstance(), 'update'), 10, 2 );
	...
	add_action('init',array(RewriteRules::getInstance(), "flushRules"));
    ...
    //include_once $arrPhysicalPath['Libs']. 'Smarty/SmartyBC.class.php';

    $objTmpl = new Smarty();
   // $objTmpl = new SmartyBC();
    ...
    if( is_admin() && (!isset($_REQUEST['ajax_in_site']))) {
       ...
       // wp_enqueue_script('oe-cw', $arrVirtualPath['Libs']. 'OE_ClientWindow/cw.js', array( 'jquery' ), Constants::CSS_JS_VERSION, true);
        require_once($arrPhysicalPath['Libs']. '/OE_ClientWindow/cw.php');
        cw::$obj = new cw(array('cookie_name'=>'cyh_cw'));
...
add_filter( 'wp_mail_from_name', 'my_mail_from_name' );
    ...
    function action_wpcf7_before_send_mail( $contact_form ) {
        ...
        if ($id){
            ...
            $data['cfid'] = 'cf_17340';
            $data['title'] = 'Seller';
            $data['your-email'] = 'test.thatsend@gmail.com';
            $data['your-subject'] = 'BrokerIDX Seller Page Request.';
            $data['your-message'] = 'BrokerIDX Seller Page Mail';

            $posted_data = serialize($data);
            echo "Test post of CF brokeridx index";
