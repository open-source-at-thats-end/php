...
if(!class_exists('RewriteRules')){
    ...
    class RewriteRules{
        ...
        private function setAllRewriteRules($matchRulePrefix){
	        ...
	        $this->IpLocationPageRewriteRules($matchRulePrefix);
            $this->SitemapPageRewriteRules($matchRulePrefix);
        }
	    function IpLocationPageRewriteRules($matchRulePrefix) {

		    global $wp_rewrite;

		    $wp_rewrite->add_rule(
			    $matchRulePrefix . '(.*)'.(ModuleUrl::getInstance()->getModuleUrl(Constants::TYPE_IP_LOCATION, false)),
			    $this->rootPageName . '&' . Constants::TYPE_URL_VAR . '=' . Constants::TYPE_IP_LOCATION ,
			    'top');
	    ...
	    function setMyAccountPageRewriteRules($matchRulePrefix)
	    {
		    ...
		    $wp_rewrite->add_rule(
			    ...
			    'top');
			     ...
			     $wp_rewrite->add_rule('pictures/property/([^/]+)/([^/]+)/?$	', 'https://looptidx.com/API/assets/assets.php?cmd=property&type=pic&MlsNum=$1&pic_no=$2', 'top');
	    }
        ...
        function SitemapPageRewriteRules($matchRulePrefix)
        {
            global $wp_rewrite;

            $wp_rewrite->add_rule(
                $matchRulePrefix  . (ModuleUrl::getInstance()->getModuleUrl(Constants::TYPE_SITEMAP,false)),
                $this->rootPageName . '&' . Constants::TYPE_URL_VAR . '=' . Constants::TYPE_SITEMAP,
                'top');
        }
