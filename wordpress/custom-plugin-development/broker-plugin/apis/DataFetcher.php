...
if(isset($_POST['api']) && $_POST['api'] == 'test'){
    $Filter = $_POST['filter'];
}
//echo '<pre>';print_r($Filter);exit();
//file_put_contents('post01.txt', print_r($_POST, true));

####### BELOW code is just for checking purpose of user access #####
/*$fname = dirname(__FILE__).'/a_log/'."API_".date("Y-m-d_H").".txt";

$refere = '';
if(isset($_SERVER['HTTP_REFERER']) && !empty($_SERVER['HTTP_REFERER']))
{
	$refere = "- REFERER :". $_SERVER['HTTP_REFERER'];
}
$content = "\n================";
$content .= "\n IP: ".$_SERVER['REMOTE_ADDR']." - USER AGENT: ".$_SERVER['HTTP_USER_AGENT']." - ACCESS TIME : ".date("Y-m-d H:i:s"). " - ACCESS URL: ".$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].$refere;
...
######## Above code is just for checking purpose of user access #########

$cache_path = $physical_path['Site_Root']."/cache-api";
$cache_filename = md5(serialize($_POST));
//echo '<pre>';echo $cache_path;exit();
$curHour = date('G');
//echo '<pre>';print_r($curHour);exit();
if($curHour >= 5)
{
...
switch($_POST['cmd'])
{
    ...
    case 'user':
        ...
        switch($_POST['type'])
        {
		    ...
		    case 'updatesavesearchemailalert':
		        ...
		        break;
            case 'getfavcondo':
                $returnData = IDXUser::getInstance()->getUserFavCondo($Filter);
                break;
            ...
            case 'getMeta':
               ...
               /*include_once($physical_path['DB_Access']. '/Metadata.php');
                ...
                foreach($Filter as $key => $val)
                {
                    ...
                    // Sub Type Actris
                    elseif($val == 'SubTypeActris')
                    {
                      $returnData['SubTypeActris'] = $asset['OL_SubType_Actris'];
                        //$returnData['SubType'] = $obIDXListing->getSubTypeKeyValueArray($Filter);
                    }
                    // Elementary School
                    ...
                    // City State
                    elseif($val == 'CityState')
                    {
                        $returnData['CityState'] = IDXListing::obj()->getCityStateKeyValueArray($Filter);
                    }
                    // County
                    elseif($val == 'County')
                    {
    ...
    case 'condo':
        ...
        {
            case 'add':
                $returnData = CondoSearch::getInstance()->addCondoSearch($Filter);
                break;
            case 'get':
                $returnData = CondoSearch::getInstance()->getCondo($Filter);
                break;
            case 'getByid':
                $returnData = CondoSearch::getInstance()->getCondoSearchById($Filter);
                break;
            case 'update':
                $returnData = CondoSearch::getInstance()->updateCondoSearch($Filter);
                break;
            case 'delete':
                $returnData = CondoSearch::getInstance()->deleteCondoSearch($Filter);
                break;
        }
        break;
    ...
    case 'condo-search':
        ...
        {
            case 'get-statistic':
                $returnData = CondoSearch::getInstance()->getCondoStatisticById($Filter);
                break;
        }
        break;
    ...
    case 'development':
        ...
        {
            case 'add':
                $returnData = Development::getInstance()->addDevelopmentPage($Filter);
                break;
            case 'get':
                $returnData = Development::getInstance()->getDevelopment($Filter);
                break;
            case 'getByid':
                $returnData = Development::getInstance()->getDevelopmentPageById($Filter);
                break;
            case 'update':
                $returnData = Development::getInstance()->updateDevelopmentPage($Filter);
                break;
            case 'delete':
                $returnData = Development::getInstance()->deleteDevelopmentPage($Filter);
                break;
            case 'getPreid':
                $returnData = Development::getInstance()->getDevelopmentPagePreId($Filter);
                break;
        }
        break;
}
