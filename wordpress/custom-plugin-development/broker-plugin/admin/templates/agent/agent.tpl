                ...
                {if $total_record > 0}
                    {foreach $rsAgent as $record}
                        <tr class="{cycle values='alt,'}">
                            ...
                            <td width="9%">{$record['agent_first_name']}</td>
                            <td width="9%">{$record['agent_last_name']}</td>
                            <td width="9%">{$record['agent_email']}</td>
                            <td width="9%">{$record['agent_phone']}</td>
                            <td width="9%">{$record['agent_website']}</td>
                            <td width="10%">{$record['agent_key_code']}</td>
                            <td width="8%">{if $record['agent_active'] == true}Yes{else}No{/if}</td>
                            <td width="9%">MARBEACHES{*{if $record['agent_mls'] == {constant('Constants::ACTRIS')}}ACTRIS{else}MARBEACHES{/if}*}</td>
                            <td width="8%">{if $record['market_report_active'] == true}Yes{else}No{/if}</td>
                            <td width="10%">
