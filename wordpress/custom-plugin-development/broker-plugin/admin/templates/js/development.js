...
function bindDisplayPage(){
    ...
    jQuery("#page_size").bind('change', function(){
        var send_data = jQuery('#frmDevelopment').serializeArray();
        send_data.push({name:'cpage',value:jQuery(this).data('cpage')});
        // send_data.push({name:'csearch_param',value:searchParam});
        send_data.push({name:'page_size',value:jQuery('#page_size').val()});
        send_data.push({name:'action',value: objdevelopmentAjax.action});
        send_data.push({name:'ajax_mod',value:'development-list-pagination'});
console.log(send_data,'bindDisplayPage');
        jQuery.jAjaxCall({
            ...
            xhr_action  :   '',
            send_data   :   send_data,
            xhr_url     :   objdevelopmentAjax.development_action,
            xhr_timeout :   (1000*10),
...
function search()
{
    jQuery('#devsearch').on('click',function () {
        var send_data = jQuery('#FrmDev').serializeArray();
        send_data.push({name:'action',value: objdevelopmentAjax.action});
        send_data.push({name:'ajax_mod',value:'development-search'});
        ...
        jQuery.jAjaxCall({
            ...
            callbefore_send: function (jqXHR, settings) {
                jQuery('#development-list-holder').html('<div class="loader-small"></div>');
            },
            callback_on_success: function(data, textStatus, jqXHR){
                bindPagination();
                bindDisplayPage();
    ...
    jQuery('#FrmDev').on('keypress',function(e) {
        if(e.which == 13) {
            var send_data = jQuery('#FrmDev').serializeArray();
            send_data.push({name:'action',value: objdevelopmentAjax.action});
            send_data.push({name:'ajax_mod',value:'development-search'});
