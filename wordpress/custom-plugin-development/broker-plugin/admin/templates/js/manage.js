jQuery(document).ready(function(){
    ...
    jQuery("select[name=sys_name]").bind('change', function(){
        getListing();
    });
    ...
    if(jQuery('#vmanage').length) {
        jQuery('.pagination ul li a').on('click', function() {
            ...
            if(jQuery(this).data('page') > 0)
            {
                ...
                send_data.push({name:'sys_name',value:jQuery("select[name=sys_name]").val()});
...
function subActionHandler(send_data){
    ...
    /*jQuery('.sys-name').on('change', function () {

        setTimeout(function () {
            jQuery(".sysName").val('yes');
        }, 800);
    });*/
    ...
    jQuery.jAjaxCall({
        ...
        callback_on_complete: function (jqXHR, textStatus) {
            /*if(jQuery(".sysName").val() == 'yes')
                jQuery(".sysName").val('no');*/
        }
    },'manag');
