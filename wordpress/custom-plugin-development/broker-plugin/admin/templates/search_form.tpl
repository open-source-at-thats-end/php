...
{*        <div id="price-range"></div>*}
    ...
    {*<pre>{$arrSearchCriteria|print_r}</pre>*}
    <div class="fholder2">
      ...
      {*  {if isset($AgentSystemName)  && $AgentSystemName == {constant('Constants::ACTRIS')}}
        ...
        {else}
            ...
            <div class="multi-opt cols3 multi-options-container">
                {*<pre>{$arrMeta['City']['MIAMI/BEACHES']|print_r}</pre>*}
                {foreach name=city from=$arrMeta['City']['MIAMI/BEACHES'] key=ckey item=citem}
                     <label><input type="checkbox" name="city[]" {if (isset($arrSearchCriteria.city)) && ($ckey|in_array:$arrSearchCriteria.city || $ckey == $arrSearchCriteria.city)}checked="checked"{else}{/if} id="{$ckey}" value="{$ckey}" />&nbsp{$citem}</label>
                {/foreach}
            </div><br>
        {*{/if}*}
    </div>
    ...
    <div class="fholder2">
        ...
        <input name="mindate" id="datepickermin" value="{if (isset($arrSearchCriteria.mindate) && $arrSearchCriteria.mindate != '')}{$arrSearchCriteria.mindate}{/if}" class="apm_monoselect datepickermin input-large">
        To
        <input name="maxdate" id="datepickermax" value="{if (isset($arrSearchCriteria.maxdate) && $arrSearchCriteria.maxdate != '')}{$arrSearchCriteria.maxdate}{/if}" class="apm_monoselect datepickermax input-large">
    </div>
    <br class="fclear">
