...
function setMapHeight(){
    ...
    if(jQuery(window).width() > 1200){
        ...
        {
            jQuery('#pms-map').css('height', (wHeight - headHeight) + 'px');
        }
        else
        {
...
function setListingHeight() {
    ...
    if(jQuery('#pms-area-listing').length > 0)
    {
        ...
        if(jQuery('#pms-map').length > 0 && jQuery('.pre-search').length > 0)
        {
            jQuery('#pms-area-listing').css('height', (wHeight - headHeight) + 'px');
        }
        else
        {
            jQuery('#pms-area-listing').css('height', (wHeight - headHeight - F_Height) + 'px');
...
//console.log(PId);
    ...
    if(total_record > 0 ) {
        ...
        /*jQuery.each(data, function (index, obj) {
            ...
            if(isGrid !== 'true' || (isGrid == 'true' && (typeof isstyle == 'undefined' || isstyle == false))){
                infoHtml += '<div id="' + obj.Key + '" class="'+mainClass+'" data-ref-id="' + obj.Key + '">';
                    infoHtml += getListingBoxHtml(obj, FavList);
                infoHtml += '</div>';
            }else{
                if(typeof isstyle !== 'undefined' && isstyle == 1)
                {
        ...
        if(typeof isPredefined != "undefined" && (isPredefined == 'true' || isPredefined == true)){
            ...
            infoHtml += '<div class="col-12 no-data-msg text-center text-danger pt-3 mt-3-">';
...
function getPage(pageNo, searchForm, mapBoundary) {
    ...
    //send_data.push({name: 'toggle-trigger-grid', prop: 'checked'});
    if(sys_name != '')
    {
    ...
    /*console.log(predefinedId > 0 && typeof tabs !== 'undefined');
