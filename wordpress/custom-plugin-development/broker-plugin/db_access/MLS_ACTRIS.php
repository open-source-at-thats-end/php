 ...
 * Time: 2:55 PM
 */
class MLS_ACTRIS extends MLSProvider
{
	public $MLSP_ID = 2;
	public $Config = array();
	...
	public function __construct($isPopulateSchema=false)
	{
		...
		# Table Info
		$this->Data['MLS_Listing'] 			= $config['Table_Prefix']. 'mls_listing_actris';
		// Temp Tables
		$this->Data['MLS_Listing_ID']		= $config['Table_Prefix']. 'mls_listing_id_actris';
		$this->Data['MLS_Open_House']		= $config['Table_Prefix']. 'mls_open_house_actris';
	...
	public function doDataTransfer_RETS2Table_API($retsResource, $retsClass, $retsSelect, $FieldList, $TableName, $property_data, $extra_field='', $extra_value='', $extra_resource=array())
	{
		global $db;
		...
		if(count($property_data) > 0)
		{
			...
			if(is_array($extra_field) && count($extra_field) > 0)
			{
				$extra_field_array = $extra_field;
				$extra_field = implode(",", $extra_field_array);
			}
			...
			foreach($property_data as $key => $data)
			{
				...
				if(is_array($data) && count($data) > 0)
				{
					...
					if(isset($data['CustomProperty']) && is_array($data['CustomProperty'])){
				...
				if(in_array("Media", $extra_field_array))
				{
					if(!is_array($extra_value))
					{
						$extra_value = array();
					}
					array_push($extra_value,serialize($Media));
				}

				if(in_array('CustomProperty', $extra_resource)){
					...
					$arr_cselect_query = explode(",",$extra_field);

					$arr_cselect_query = array_fill_keys($arr_cselect_query, '');

					$cusListing = array_intersect_key($customProperty[0], $arr_cselect_query);

					$cus_extra_value = array_values($cusListing);

					if(is_array($cus_extra_value) && count($cus_extra_value) > 0)
					{
						$extra_value = $extra_value + $cus_extra_value;
					}
				}

				//$extra_value = implode("', '",array_map( 'addslashes',$extra_value));

				//$extra_value = addslashes($extra_value);
				if(is_array($extra_value) && count($extra_value) > 0){
					$extra_value = implode("', '", $extra_value);
				}
	...
	function TransferListingToMst()
	{
		...
		for($i=0; $i<=$TotalListing; $i=$i+$page_size)
		{
			...
			$sql =	" INSERT INTO ".$this->Data['TableName'].
				...
				" TotalUnits, TotalRooms, Buildings, Tax, Elementary_School, High_School, Middle_School, School_District, LastUpdateDate, Listing_Created_Date, is_mark_for_deletion, Is_ShortSale, Is_REO, SystemName)".
				" SELECT ".$this->MLSP_ID.", MLS_NUM, Agent_ID, Agent_ShortID, OfficeID, CoAgent_ID, '', '', '',BuyAgent_ID,CoBuyAgent_ID, ListingKey,".
				...
				" IF(TotalUnits > 0, TotalUnits, 0), IF(TotalRooms > 0, TotalRooms, 0), IF(Buildings > 0, Buildings, 0), Tax, Elementary_School, High_School, Middle_School, '', LastUpdateDate, NOW(), 'N', IF(Is_ShortSale = 1, 'Yes', 'No'), IF(Is_REO = 1, 'Yes', 'No'),SystemName".
				...
				" Parking = VALUES(Parking), Garage = VALUES(Garage), Stories = VALUES(Stories), Description = VALUES(Description), Levels = VALUES(Levels),".
				...
				" Baths = VALUES(Baths), LastUpdateDate = VALUES(LastUpdateDate), LastPhotoDate = VALUES(LastPhotoDate), is_mark_for_deletion = VALUES(is_mark_for_deletion), Is_ShortSale = VALUES(Is_ShortSale), Is_REO = VALUES(Is_REO) ,SystemName = VALUES(SystemName)";
			$db->query($sql);
			...
			$sql =	" INSERT INTO ".$this->Data['Listing_Address'].
				...
				" State, StateName, County, Subdivision, StreetNumberModifier, StreetSuffixModifier, process_flag, entry_date, Latitude, Longitude, MLSP_ID)".
				...
				" State, '', County, REPLACE(Subdivision,'&#xA4;',''), '', '', 'N', UNIX_TIMESTAMP( ), Latitude, Longitude, ".$this->MLSP_ID.
				" FROM ".$this->Data['MLS_Listing'].
				" LIMIT ".$i.", ".$page_size.
	...
	public function downloadMetaData($MLSP_ID)
	{
		...
		// check for errors
		if (MLS_ACTRIS::obj()->Config['MLS_IS_API'] == 'Yes' && !empty(MLS_ACTRIS::obj()->Config['MLS_Client_ID']) && !empty(MLS_ACTRIS::obj()->Config['MLS_Client_Secret'])  && !empty(MLS_ACTRIS::obj()->Config['MLS_Browser_Token'])) {
			echo "  + Connected<br>\n";
		}
		else {
		...
		$result = Utility::readUrl($url, '', '', '', MLS_ACTRIS::obj()->Config['MLS_Browser_Token']);

		//MLS_BRIDGE::obj()->MLSP_ID
		// Save response to XML file
		$xmlFile = $physical_path['Cronjob'].'/'.$this->picPath['MLS_Pic_Folder'][MLS_ACTRIS::obj()->MLSP_ID].'/metadata.xml';
