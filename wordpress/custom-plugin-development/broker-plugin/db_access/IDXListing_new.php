	...
	public function getAddressSuggestion($POST)
	{
		...
		if(trim($POST['keywords']) != '')
		{
			...
			if($POST['type'] == 'MLS' || $POST['type'] == '')
			{
				...
				$sql = "SELECT DISTINCT(`mls_num`) AS MLS_NUM, `listing_status` AS ListingStatus FROM trigger_search_by_mls WHERE 1 ".$addParameters." ORDER BY `mls_num` ASC LIMIT 0," . $POST['Limit'];

				$rs = $db->quick_query($sql);
				$ars = $rs->quick_fetch_all();
				foreach ($ars as $k => $v)
					$arr[] = array('type' => ASTYPE_MLS, 'title' => $v['MLS_NUM'], 'Status' => $v['ListingStatus']);
